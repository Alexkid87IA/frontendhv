...
const authors = [
  ...
  },
  {
    ...
    email: "alexia@rogerormieres.fr",
    twitter: "@alexiadelvaux",
    linkedin: "alexiadelvaux",
  ...
  },
  {
    ...
    email: "victor@rogerormieres.fr",
    twitter: "@victorsorel",
    linkedin: "victorsorel",
...
const createAuthors = async () => {
  try {
    console.log('Starting authors creation...');
    console.log('API URL:', env.VITE_STRAPI_API_URL);
    
    for (const author of authors) {
      console.log(`Creating author: ${author.name}`);
      const response = await strapiApi.post('/authors', { data: author });
      console.log(`Author ${author.name} created:`, response.data);
    }
